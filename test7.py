...
def process_cv_file(cv_file_path):
    ...
    un JSON structuré. Retourne un dict fusionné (file_name + contenu IA).
    """
    ...
    # Prétraitement (vous pouvez décider d'envoyer brut ou pré-traité)
    ...
    if analysis and isinstance(analysis, dict):
        # On fusionne file_name + champs du JSON IA
        final_json = {
            "file_name": os.path.basename(cv_file_path)
        }
        final_json.update(analysis)  
        # final_json aura : {"file_name": "...", "candidat": {...}, "cv": {...}}
        return final_json
    else:
        # Si pas de JSON valide, on peut stocker le texte ou ignorer
        # Ici on choisit de l'ignorer (None => pas ajouté dans results)
        return None
